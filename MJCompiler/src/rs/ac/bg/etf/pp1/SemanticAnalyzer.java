...
public class SemanticAnalyzer extends VisitorAdaptor {
	...
	public void visit(TermTarOp termTarOp) {
		if (termTarOp.getTerm().struct == Tab.intType && termTarOp.getTermTar().struct == Tab.intType)
			...
			report_error("Greska: Operacija # nedozvoljenih tipova", termTarOp);
		}
	}

	public void visit(FactorAtOp factorAtOp) {
		if (factorAtOp.getFactorAt().struct == Tab.intType)
			factorAtOp.struct = Tab.intType;
		else {
			factorAtOp.struct = Tab.noType;
			report_error("Greska: Operacija ^ nedozvoljenog tipa", factorAtOp);
		}
	}
	...
	public void visit(SingleAtFactor singleAtFactor) {
		singleAtFactor.struct = singleAtFactor.getFactorAt().struct;
	}
	
	public void visit(FactorArray factorArray) {
		lastFactorObj = null;
		//Obj arrayObj = Tab.find(factorArray.getName());		
		if(factorArray.getDesignator().obj.getType().getKind() == Struct.Array && 
				factorArray.getDesignator().obj.getType().getElemType() == Tab.intType)
			factorArray.struct = Tab.intType;
		else {
			factorArray.struct = Tab.noType;
			report_error("Greska: Operacija @ nedozvoljenog tipa", factorArray);
		}
	}
